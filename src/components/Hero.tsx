    ...
    <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6 text-center">
      <AnimatePresence>
        {isIntroRunning && (
          <motion.button
            ...
            className="absolute bottom-10 right-6 z-20 text-sm text-faint transition hover:text-text sm:bottom-12 sm:right-8"
          >
            {content.skipLabel}
          </motion.button>
        )}
      ...
      <div className="relative z-10 flex min-h-[16rem] w-full max-w-4xl flex-col items-center justify-center pt-28 sm:pt-32">
        ...
        >
          ...
          <p className="mt-3 flex h-10 items-center font-display text-2xl font-semibold text-text sm:text-4xl">
            <span>{typed}</span>
        ...
        {showLargeWord && (
          ...
          >
            <div className="flex items-baseline justify-center font-display text-6xl font-extrabold tracking-tight sm:text-8xl">
              <motion.span
                transition={{ duration: 0.55, ease: "easeInOut" }}
                animate={
                  showD
                    ? { color: "#dc7b40", scale: [1, 1.18, 1] }
                    : { color: "#a39682", scale: 1 }
                }
          ...
          {showContent && (
            ...
            >
              <p className="max-w-2xl text-balance text-base leading-relaxed text-muted sm:text-lg">
                ...
                <span className="font-semibold text-text">{content.summary.highlight}</span>
                {content.summary.suffix}
              </p>
              <button
                type="button"
                onClick={onDemoClick}
                className="mt-8 rounded-full bg-brand px-8 py-4 text-base font-semibold text-white shadow-lg shadow-brand/30 transition hover:scale-[1.03] hover:bg-brand-deep"
              >
                {content.cta}
              </button>
              <motion.div
                ...
                className="mt-16 text-sm text-faint"
              >
